...
public class SecurityLoginLogic : BaseLogic<SecurityLoginPoco>
{
    ...
    protected override void Verify(SecurityLoginPoco[] pocos)
    {
        ...
        foreach (var poco in pocos)
        {
            ...
            {
                exceptions.Add(new ValidationException(700, $"Password for SecurityLogin {poco.Password} cannot be null"));
            }
            else if (poco.Password.Length < 10)
            {
                exceptions.Add(new ValidationException(700, $"Password for SecurityLogin {poco.Password} must be at least 10 characters."));
            }
            else if (!requiredExtendedPasswordChars.Any(t => poco.Password.Contains(t)))
            {
                exceptions.Add(new ValidationException(701, $"Password for SecurityLogin {poco.Password} must contain an extended character of '$', '*', '#', '_' or '@' ."));
            }

            if (string.IsNullOrEmpty(poco.PhoneNumber))
            {
                exceptions.Add(new ValidationException(702, $" {poco.PhoneNumber} is required"));
            }
            else
            {
                ...
                {
                    exceptions.Add(new ValidationException(703, $" {poco.PhoneNumber} is not in the required format."));
                }
                else
                {
            ...
            {
                exceptions.Add(new ValidationException(704, $" {poco.EmailAddress} is not a valid email address format."));
            }
            ...
            {
                exceptions.Add(new ValidationException(705, $" {poco.FullName} is required for SecurityLogin."));
            }
