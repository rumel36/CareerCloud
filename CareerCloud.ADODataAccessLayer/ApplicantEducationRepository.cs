...
namespace CareerCloud.ADODataAccessLayer
{
    public class ApplicantEducationRepository : IDataRepository<ApplicantEducationPoco>
    {
        ...
        public ApplicantEducationRepository() {
            ...
            var config = new ConfigurationBuilder(); //take instance for building the the path of DB
            var path = Path.Combine(Directory.GetCurrentDirectory(), "appsettings.json");// Get the current directory of application and combine it
            config.AddJsonFile(path, false);
            var root = config.Build();
            _connStr = root.GetSection("ConnectionStrings").GetSection("DataConnection").Value;//So, when the instance creates, the db connection is established
        ...
        public ApplicantEducationPoco GetSingle(
            Expression<Func<ApplicantEducationPoco, 
                bool>> where, 
                   params Expression<Func<ApplicantEducationPoco, object>>[] navigationProperties)
        {
